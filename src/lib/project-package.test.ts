...
describe(".barun 프로젝트 패키지", () => {
  it("프로젝트 데이터와 첨부파일을 한 파일에 담고 절대 작업 경로는 제외한다", async () => {
    ...
    project.expenses = [{
      ...
      attachments: [{ id: "receipt", relativePath: "attachments/receipt.png", originalName: "receipt.png", mimeType: "image/png", kind: "online-receipt", layout: { scale: 1.35, offsetX: 12, offsetY: -7, rotation: 90 } }],
      itemDetails: "",
      isFuel: false,
      paymentSource: "team",
    ...
    expect(parsed.project.expenses[0].attachments[0].layout).toEqual({ scale: 1.35, offsetX: 12, offsetY: -7, rotation: 90 });
