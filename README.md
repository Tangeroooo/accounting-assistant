...
- 영수증 이미지는 파일 선택과 클립보드 붙여넣기(`⌘V`/`Ctrl+V`)를 모두 지원합니다. PDF의 모든 페이지는 로컬에서 이미지로 바꾸고, 아이폰 HEIC·HEIF 사진은 원본을 그대로 보존한 채 편집·출력용 고화질 JPEG와 화면용 축소 미리보기를 함께 만듭니다. 편집 화면에서는 가벼운 미리보기를 사용하고 PDF·Word 출력에는 고화질 이미지를 사용합니다. 지출 등록·수정 화면의 위·아래 버튼으로 여러 첨부파일의 출력 순서를 바꿀 수 있습니다.
- 오프라인 영수증은 지출 입력에서부터 부착 공간을 여러 개 만들고 각각의 너비·높이를 정할 수 있습니다. 영수증철에는 현재 가로·세로 cm를 반투명 워터마크로, 실제 영수증 크기에 맞추라는 설명을 눈에 띄는 안내로 표시합니다. 큰 영수증은 접지 않고, 잘라 붙일 조각 수만큼 영수증 개수를 추가해 `1-1`, `1-2`, `1-3`처럼 같은 영수증의 일련번호로 구분합니다.
- 날짜·금액 노란색 표시와 금액 옆 영수증 번호 표시는 출력 후 사용자가 직접 합니다.
- 교통비의 주유비 산정 증빙은 개별 영수증과 1:1로 묶지 않고 프로젝트 공통 자료로 관리합니다. 온라인 파일과 인쇄 후 붙일 오프라인 부착칸을 각각 여러 개 추가할 수 있으며, 영수증철에서는 날짜가 더 빠른 일반 교통비 뒤 첫 주유비 지출 바로 앞에 배치합니다.
- 수입·지출·자동 검토를 한 화면에 표시하며 지출 입력 중에도 수입 − 지출 − 환입액 차액을 실시간 계산합니다. 수입 관리와 자동 검토는 접어 요약만 남길 수 있고, 검산 차액은 지출표를 스크롤하는 동안에도 상단에 고정됩니다.
- 지출의 `내용` 입력란 아래에는 원본 템플릿에서 가져온 항목별 상세 작성 예시를 여러 개 고정 표시합니다. 여행자·단체보험의 보험가입증명서 누락은 저장을 막지 않는 `주의`로 안내합니다.
- 지출 등록·수정의 `내역 반영` 버튼이 비활성화되면 버튼 바로 옆 상태 안내에 빠진 날짜·내용·금액·개인 결제자 이름만 실시간으로 표시합니다.
...
`.barun`은 `manifest.json`과 영수증·증빙 이미지, 필요한 렌더링·미리보기 이미지를 한 파일에 담는 ZIP 기반 문서 패키지입니다. 다른 컴퓨터나 브라우저에서는 `.barun` 파일 하나만 옮겨 열 수 있습니다. 이미 압축된 이미지 파일은 ZIP에서 다시 압축하지 않아 대용량 프로젝트의 저장 지연을 줄입니다. 웹앱은 자동 복구 때마다 `.barun` 전체를 다시 만들지 않고 프로젝트 정보와 첨부 자산을 브라우저 저장소에 나누어 갱신합니다. 브라우저 데이터를 지우기 전 중요한 시점마다 `.barun` 백업을 내려받아야 합니다. 데스크톱 앱은 같은 폴더의 임시 파일을 완성한 뒤 기존 파일과 교체하며 업데이트 직전에는 `프로젝트명-업데이트전-백업.barun`도 만듭니다. 자세한 형식은 [docs/barun-project-format.md](docs/barun-project-format.md)를 참고하세요. 기존 `회계프로젝트.json`은 열기만 지원하며 다음 저장부터 `.barun`으로 전환합니다.
...
배포하는 `*-setup.exe`는 NSIS 설치 프로그램입니다. 설치가 끝나면 Windows `설정 → 앱 → 설치된 앱`에 아웃리치 회계와 제거 항목이 등록되므로 별도의 언인스톨러 파일을 내려받을 필요가 없습니다.
