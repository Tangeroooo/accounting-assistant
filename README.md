...
- 영수증철은 A4 세로 한 장에 최대 6개를 세로 우선으로 배치합니다. 지출 번호·내용은 편집 화면에서만 확인하고 PDF에는 넣지 않으며, 오프라인 영수증은 중앙의 작은 빈 점선 위에 실물을 붙입니다.
- 온라인 영수증은 편집 화면에서 이동·확대·축소·회전·자르기가 가능하고, 각 배치 칸에서 잘라 다른 영수증과 겹치지 않게 PDF로 저장합니다.
- 영수증 이미지는 파일 선택과 클립보드 붙여넣기(`⌘V`/`Ctrl+V`)를 모두 지원합니다.
...
`.barun`은 `manifest.json`과 영수증·증빙 원본을 한 파일에 담는 ZIP 기반 문서 패키지입니다. 다른 컴퓨터에서는 `.barun` 파일 하나만 옮겨 열 수 있습니다. 자세한 형식은 [docs/barun-project-format.md](docs/barun-project-format.md)를 참고하세요. 기존 `회계프로젝트.json`은 열기만 지원하며 다음 저장부터 `.barun`으로 전환합니다.
