...
- 영수증 이미지는 파일 선택과 클립보드 붙여넣기(`⌘V`/`Ctrl+V`)를 모두 지원합니다. PDF를 첨부하면 한글 CID 글꼴을 포함해 모든 페이지를 PNG로 바꿔 편집하고 `.barun`에도 변환된 이미지만 저장합니다. 지출 등록·수정 화면의 위·아래 버튼으로 여러 첨부파일의 출력 순서를 바꿀 수 있습니다.
