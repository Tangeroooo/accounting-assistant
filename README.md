...
- 오프라인 영수증은 지출 입력에서부터 부착 공간을 여러 개 만들고 각각의 너비·높이를 정할 수 있습니다. 영수증철에는 현재 가로·세로 cm와 실제 영수증 크기에 맞추라는 안내를 표시합니다. 큰 영수증은 접지 않고, 잘라 붙일 조각 수만큼 영수증 개수를 추가해 `1-1`, `1-2`, `1-3`처럼 같은 영수증의 일련번호로 구분합니다.
...
- 지출 등록·수정의 `내역 반영` 버튼이 비활성화되면 하단 상태 안내에 빠진 날짜·내용·금액·개인 결제자 이름만 실시간으로 표시합니다.
